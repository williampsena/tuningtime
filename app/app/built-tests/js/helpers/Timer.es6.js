...
var Timer = function () {
  ...
  _createClass(Timer, [{
    ...
    key: 'setCurrentTimer',
    value: function setCurrentTimer(model) {
      _Session.SessionHelper.create(TIMER_SESSION_KEY, model);
    }
  }, {
    key: 'removeCurrentTimer',
    value: function removeCurrentTimer() {
      _Session.SessionHelper.remove(TIMER_SESSION_KEY);
    }
  }, {
    key: 'getCurrentTimer',
    value: function getCurrentTimer() {
      var timer = _Session.SessionHelper.get(TIMER_SESSION_KEY);

      if (timer) {
        timer = new Timer(timer.counter);
      }
    ...
    value: function setCurrentTimerHistory(model) {
      _Session.SessionHelper.create(TIMER_HISTORY_SESSION_KEY, model);
    }
  }, {
    key: 'removeCurrentTimerHistory',
    value: function removeCurrentTimerHistory() {
      _Session.SessionHelper.remove(TIMER_HISTORY_SESSION_KEY);
    }
  }, {
    key: 'getCurrentTimerHistory',
    value: function getCurrentTimerHistory() {
      var timerHistory = _Session.SessionHelper.get(TIMER_HISTORY_SESSION_KEY);

      if (timerHistory) {
        timerHistory = new TimerHistoryModel(timer);
      }
